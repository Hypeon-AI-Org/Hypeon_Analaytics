    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-4 items-center">
        ...
        <button
          type="button"
          onClick={() => load()}
          className="rounded bg-indigo-600 text-white px-3 py-1 text-sm hover:bg-indigo-700"
        >
          Refresh
        </button>
      </div>
      ...
      {error && (
        <div className="rounded bg-red-50 text-red-700 px-4 py-2 text-sm" role="alert">
          {error}
        </div>
      )}
      ...
      {!loading && items.length > 0 && (
        <div className="bg-white shadow rounded-lg overflow-hidden">
          <ul className="divide-y divide-gray-200">
            {items.map((insight) => (
              <li key={insight.insight_id} className="p-4">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div className="flex-1 min-w-0">
                    ...
                    {expandedId === insight.insight_id && (
                      <div className="mt-2 text-sm text-gray-600 rounded bg-gray-50 p-2">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" role="dialog" aria-modal="true" aria-labelledby="simulate-title">
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4 p-6">
        <h2 id="simulate-title" className="text-lg font-semibold text-gray-900 mb-4">Simulate budget shift</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="from-campaign" className="block text-sm font-medium text-gray-700">From campaign</label>
            <input
              id="from-campaign"
              type="text"
              value={fromCampaign}
          ...
          <div>
            <label htmlFor="to-campaign" className="block text-sm font-medium text-gray-700">To campaign</label>
            <input
              id="to-campaign"
              type="text"
              value={toCampaign}
          ...
          <div className="flex gap-2 justify-end">
            ...
            <button type="submit" disabled={loading} className="rounded bg-indigo-600 text-white px-4 py-2 text-sm hover:bg-indigo-700 disabled:opacity-50">
